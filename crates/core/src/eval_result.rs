...
mod tests {
    ...
    fn matched_true_when_any_matched() {
        let r =
            EvaluationResult::new(vec![result(AuditType::Unmatched), result(AuditType::Matched),]);
        assert!(r.matched());
    }
    ...
    fn matched_false_when_none_matched() {
        let r =
            EvaluationResult::new(vec![result(AuditType::Unmatched), result(AuditType::Errored),]);
        assert!(!r.matched());
    }
    ...
    fn verdict_unmatched_when_all_unmatched() {
        let r = EvaluationResult::new(vec![result(AuditType::Unmatched),
