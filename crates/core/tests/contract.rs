...
fn audit_id_matches_java_format() {
    ...
    assert_eq!(audit_id("source-events", 3, 42, "rule-7"),
               "source-events:3:42:rule-7");
}

#[test]
fn audit_type_serializes_screaming_snake() {
    assert_eq!(serde_json::to_string(&AuditType::Matched).unwrap(),
               "\"MATCHED\"");
    assert_eq!(serde_json::to_string(&AuditType::Unmatched).unwrap(),
               "\"UNMATCHED\"");
    assert_eq!(serde_json::to_string(&AuditType::Errored).unwrap(),
               "\"ERRORED\"");
    assert_eq!(serde_json::from_str::<AuditType>("\"ERRORED\"").unwrap(),
               AuditType::Errored);
}

#[test]
fn audit_record_uses_camel_case_keys() {
    let rec = AuditRecord { audit_id: audit_id("source-events", 0, 5, "r1"),
                            rule_id: "r1".into(),
                            schema_version: 1,
...
fn rule_round_trips() {
    ...
    assert!(serde_json::to_value(&rule).unwrap()
                                       .get("targetTopic")
                                       .is_some());
}
...
fn evaluation_result_verdict_precedence() {
    let r = |t| RuleResult { rule_id: "x".into(),
                             audit_type: t,
                             reason: None };
    ...
    // empty -> UNMATCHED
    assert_eq!(EvaluationResult::new(vec![]).verdict(),
               AuditType::Unmatched);
}
...
fn json_value_passthrough() {
    ...
    assert_eq!(serde_json::from_str::<serde_json::Value>(&v.to_string()).unwrap(),
               v);
}
