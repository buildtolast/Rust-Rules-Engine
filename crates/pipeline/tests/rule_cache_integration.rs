...
async fn load_returns_only_enabled_rules() {
    ...
    let _: sqlx::postgres::PgQueryResult = sqlx::query("TRUNCATE rules").execute(&pool)
                                                                        .await
                                                                        .expect("truncate");
    ...
    repo.create(&RuleInput { description: "enabled 1".into(),
                             expression: "event.x > 1".into(),
                             target_topic: "t".into(),
                             enabled: true })
        .await
        .unwrap();
    repo.create(&RuleInput { description: "enabled 2".into(),
                             expression: "event.y == \"a\"".into(),
                             target_topic: "t".into(),
                             enabled: true })
        .await
        .unwrap();
    repo.create(&RuleInput { description: "disabled".into(),
...
async fn hot_reload_swaps_on_notify() {
    ...
    tokio::spawn(async move {
        watch_and_reload(cache_clone, repo_clone, listener).await
                                                           .ok();
    });
    ...
    repo.update(first_id,
                &RuleInput { description: "disabled".into(),
...
async fn bad_rule_expression_is_skipped_not_fatal() {
    ...
    repo.create(&RuleInput { description: "good rule".into(),
                             expression: "event.x > 1".into(),
                             target_topic: "t".into(),
                             enabled: true })
        .await
    ...
    repo.create(&RuleInput { description: "bad rule".into(),
    ...
    let cache = RuleCache::load(&repo).await
