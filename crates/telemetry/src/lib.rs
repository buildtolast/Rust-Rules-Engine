...
pub fn init(service_name: &str) -> ShutdownGuard {
    ...
    let resource = Resource::new(vec![opentelemetry::KeyValue::new("service.name",
    ...
    let log_exporter = LogExporter::builder().with_tonic()
                                             .with_endpoint(&endpoint)
                                             .build()
                                             .expect("OTLP log exporter build failed");

    let logger_provider = LoggerProvider::builder().with_batch_exporter(log_exporter,
                                                                        runtime::Tokio)
