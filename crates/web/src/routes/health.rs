...
async fn check_postgres(s: &AppState) -> Check {
    ...
    match timeout(Duration::from_secs(2), s.rules.ping()).await {
        Ok(true)  => Check { ok: true,  latency_ms: t.elapsed().as_millis() as u64, error: None },
        Ok(false) => Check { ok: false, latency_ms: t.elapsed().as_millis() as u64, error: Some("ping query failed".into()) },
        Err(_)    => Check { ok: false, latency_ms: 2000, error: Some("timeout after 2s".into()) },
    }
}
...
async fn check_kafka(s: &AppState) -> Check {
    ...
    let broker = s.kafka_brokers.split(',').next().unwrap_or("").trim().to_string();
    match timeout(
        Duration::from_secs(2),
        tokio::net::TcpStream::connect(&broker),
    ).await {
        Ok(Ok(_))  => Check { ok: true,  latency_ms: t.elapsed().as_millis() as u64, error: None },
        Ok(Err(e)) => Check { ok: false, latency_ms: t.elapsed().as_millis() as u64, error: Some(e.to_string()) },
        Err(_)     => Check { ok: false, latency_ms: 2000, error: Some("timeout after 2s".into()) },
    }
}
