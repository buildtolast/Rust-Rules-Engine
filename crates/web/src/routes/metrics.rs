...
async fn query_clickhouse(s: &AppState) -> ClickHouseMetrics {
    ...
    struct Row { #[serde(rename = "n")] n: u64 }

    let (audits, sre) = tokio::join!(
        s.ch_client.query("SELECT count() AS n FROM audits").fetch_one::<Row>(),
        s.ch_client.query("SELECT count() AS n FROM sre_observations").fetch_one::<Row>(),
    );
...
async fn query_postgres(s: &AppState) -> PostgresMetrics {
    ...
    #[derive(sqlx::FromRow)]
    struct Counts { total: i64, enabled: i64 }
    ...
    match result {
        ...
        Err(_) => PostgresMetrics { rules_total: 0, rules_enabled: 0, latency_ms },
    }
}
