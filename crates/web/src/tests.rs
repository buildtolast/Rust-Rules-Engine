...
mod tests {
    ...
    async fn test_health_returns_200() {
        let response = call(health_only_router(),
    ...
    async fn test_unknown_route_returns_404() {
        let response = call(health_only_router(),
                            Request::builder().uri("/does-not-exist")
                                              .body(Body::empty())
                                              .unwrap()).await;
    ...
    async fn try_build_app_state() -> Option<crate::AppState> {
        ...
        let producer: rdkafka::producer::FutureProducer =
            ClientConfig::new().set("bootstrap.servers", &kafka_brokers)
    ...
    async fn test_rules_list_returns_200_with_array() {
        test_support::skip_if_unavailable!(async {
        ...
        let response = call(crate::router(state, vec![]),
                            Request::builder().uri("/api/rules")
                                              .body(Body::empty())
                                              .unwrap()).await;
        assert_eq!(response.status(), StatusCode::OK);
        let bytes = collect_body(response.into_body()).await;
    ...
    async fn test_create_rule_returns_201_and_retrieve_returns_200() {
        ...
        let create_response = call(app.clone(),
        ...
        // GET /api/rules/:id
        let get_response = call(app,
                                Request::builder().uri(format!("/api/rules/{id}"))
                                                  .body(Body::empty())
                                                  .unwrap()).await;
        ...
        assert_eq!(fetched["description"].as_str(),
    ...
    async fn test_get_nonexistent_rule_returns_404() {
        ...
        let response =
            call(crate::router(state, vec![]),
                 Request::builder().uri("/api/rules/00000000-0000-0000-0000-000000000000")
                                   .body(Body::empty())
                                   .unwrap()).await;
        assert_eq!(response.status(), StatusCode::NOT_FOUND);
    }
    ...
    async fn test_analytics_stats_returns_200_with_object_body() {
        ...
        let response = call(crate::router(state, vec![]),
                            Request::builder().uri("/api/analytics/stats")
                                              .body(Body::empty())
                                              .unwrap()).await;
        assert_eq!(response.status(), StatusCode::OK);
        let bytes = collect_body(response.into_body()).await;
    ...
    async fn test_readiness_probe_returns_valid_shape() {
        ...
        let response = call(crate::router(state, vec![]),
                            Request::builder().uri("/health/ready")
                                              .body(Body::empty())
                                              .unwrap()).await;
    ...
    async fn test_export_rejects_unknown_audit_type() {
        ...
        let response = call(crate::router(state, vec![]),
                            Request::builder().uri("/api/reports/export?type=BOGUS")
                                              .body(Body::empty())
                                              .unwrap()).await;
