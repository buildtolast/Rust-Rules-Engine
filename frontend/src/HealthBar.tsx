    ...
    <div className="flex items-center gap-4 px-6 py-2 bg-gray-50 border-b border-gray-100 text-xs">
      ...
      {services.length === 0 ? (
        ...
        services.map(([name, svc]) => (
          <div key={name} className="flex items-center gap-1.5" title={svc.error ?? `${svc.latency_ms}ms`}>
            <span className={`w-2 h-2 rounded-full flex-shrink-0 ${svc.status === 'ok' ? 'bg-emerald-500' : 'bg-red-500 animate-pulse'}`} />
            <span className={`font-medium ${svc.status === 'ok' ? 'text-gray-600' : 'text-red-600'}`}>
